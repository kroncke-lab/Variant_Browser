...
def datatables_api(request):
    ...
    try:
        ...
        logger.info("SCN5A API ok: rows=%s duration=%.2fs", len(data), duration_s)
        ...
        duration_s = time.monotonic() - start
        logger.exception("SCN5A API failed after %.2fs", duration_s)
        return JsonResponse({'error': 'SCN5A API error'}, status=500)
